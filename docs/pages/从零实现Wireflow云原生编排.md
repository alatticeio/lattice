...
Lattice 的出发点是：把 WireGuard 节点当作 Kubernetes 资源来声明，用 Operator 来编排，用 NATS 来信令，用   
ICE 来穿透。
...
IP 分配是 Overlay 网络编排最容易出错的环节。Lattice 使用两级分配模型，完全依托 Kubernetes
资源的原子性来避免竞态。

4.1 第一级：子网分配

控制平面从 LatticeGlobalIPPool（如 10.10.0.0/16）中为每个网络分配一个 /24 子网：

func AllocateSubnet(networkName, globalPool string) (string, error) {                                      
// 枚举所有 LatticeSubnetAllocation，收集已用子网    
used := collectUsedSubnets()
      ...
      for _, subnet := range enumerate(globalPool, 24) {                                                     
          ...
          err := k8sClient.Create(ctx, &LatticeSubnetAllocation{                                            
...
func AllocateIP(networkCIDR string, peer *LatticePeer) (string, error) {                                  
used := collectUsedEndpoints(namespace)
      ...
      for _, ip := range hosts(networkCIDR) {                                                                
          ...
          err := k8sClient.Create(ctx, &LatticeEndpoint{
              ObjectMeta: metav1.ObjectMeta{
...
func (r *PeerReconciler) reconcileJoinNetwork(ctx context.Context, peer *LatticePeer) (ctrl.Result, error)
{
// 1. 生成密钥对（幂等：已有则跳过）
...
Lattice 没有引入独立的消息队列服务，而是在 latticed 进程内直接嵌入 NATS Server：
...
lattice.signals.peer              ← Agent 注册/上报                                                       
lattice.signals.peers.{peerId}    ← 点对点配置下发                                                        
lattice.signals.peers.{peerId}.ice ← ICE candidate 交换
...
WireGuard 的 IP/路由操作在各平台命令不同，Lattice 用接口隔离：
...
# config/lattice/overlays/production/kustomization.yaml
resources:                                                                                                 
- ../../base
...
WireGuard 的简洁是其内核层面的美德，但编排层的复杂性不会消失，只是被转移到了别处。Lattice

#!/bin/bash
# Lattice Bootstrapper — runs inside lattice/installer container
# Usage:
#   docker run --rm -it \
...
#   CLUSTER_NAME          — override cluster name (default: lattice)
#   API_PORT              — override API port   (default: 8080)
#   NATS_PORT             — override NATS port  (default: 4222)
#   GITHUB_RAW            — override manifest base URL
#
# NOTE: This installer deploys all-in-one mode only (latticed with embedded NATS + SQLite).
# For separated/production deployments: kubectl apply -k config/lattice/overlays/production
set -euo pipefail

# ═══════════════════════════ Config ═══════════════════════════
CLUSTER_NAME="${CLUSTER_NAME:-lattice}"
NAMESPACE="lattice-system"
API_PORT="${API_PORT:-8080}"
NATS_PORT="${NATS_PORT:-4222}"
HEALTH_TIMEOUT="${HEALTH_TIMEOUT:-120}"
GITHUB_RAW="${GITHUB_RAW:-https://raw.githubusercontent.com/alatticeio/lattice/master}"
...
echo ""
echo "┌──────────────────────────────────────────────────────────┐"
echo "│         🌊  Lattice  Bootstrapper (Docker mode)         │"
echo "└──────────────────────────────────────────────────────────┘"
...
# ─────────────────────────────────────────────────────────────
step "Step 2  Deploying Lattice control plane (all-in-one)"

info "Applying CRDs..."
kubectl apply -f "${GITHUB_RAW}/deploy/quickstart/lattice-crds.yaml" >/dev/null
ok "CRDs applied"
...
# STEP 4  Health check
# ─────────────────────────────────────────────────────────────
step "Step 3  Waiting for Lattice to become ready"

info "Waiting for latticed pod (timeout ${HEALTH_TIMEOUT}s)..."
kubectl wait --for=condition=Ready pod \
    -l app.kubernetes.io/name=latticed \
    -n "${NAMESPACE}" \
...
if $HEALTH_OK; then
    ...
    warn "API not yet responding. Check: kubectl logs -l app=latticed -n ${NAMESPACE}"
fi
...
printf "      -e CLEANUP=true lattice/installer:latest\n"
echo ""
